...
public class CursoEntity {
	...
	private Set<MateriasEntity> materias = new HashSet<>();
	...
	public void fromCurso(Curso curso) {
		...
		this.materias = curso.getMaterias().stream().map(MateriasEntity::new).collect(Collectors.toSet());
	}

	public Curso toCurso() {
		return new Curso(this.getReferencia(), this.getNombre(), this.getMaterias().stream()
				.map(materiaEntity -> materiaEntity.toMateria()).collect(Collectors.toSet()));
	}
	...
	public void addMateria(MateriasEntity materia) {
